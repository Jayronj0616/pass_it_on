...
- **`app/login/page.tsx` and `app/signup/page.tsx`** — both previously had zero way to navigate anywhere except toggling between login/signup or back to `/` via the logo. User pointed this out as a real gap. Fixed with a minimal, deliberately-scoped addition (not the full nav, not the footer — user explicitly chose "minimal way back to browsing" to preserve the focused-auth pattern): a single "Browse without an account" link under the card, pointing to `/browse`. **Superseded by §19 (2026-09-11)** — the user reversed this decision and asked for a real header/footer on these pages after all.
...
## 19. Shared header/footer on auth + browsing pages — DONE, tsc clean, lint clean — 2026-09-11

Reverses the §12a decision to keep `/login` and `/signup` minimal. The user's reasoning this time: those pages, plus `/`, plus the whole `(app)` group (`/browse`, `/dashboard/*`, `/items/*`, `/messages`, `/profile`) should look like "most of the website" — a real header and a real multi-column footer — not a bare card or a header with no matching footer. Referenced a screenshot of an unrelated site's footer (columns for nav/services/contact + a bottom copyright bar) as the shape to aim for, adapted here to PassItOn's actual pages only (no invented links, same rule as the original `LandingFooter`).

What changed:

- **New `components/nav/SiteFooter.tsx`** — replaces `components/landing/LandingFooter.tsx` (deleted, now unused). Four columns (brand blurb, Navigation, Account, Legal) plus a copyright bar. Used on every page listed above.
- **New `components/nav/PublicHeader.tsx`** — a real bordered header bar for `/login` and `/signup` (logo linking home + a Browse link), replacing the floating "PassItOn" text link. Deliberately lighter than the `(app)` group's header (no Log in/Sign up links, since the page itself already is that).
- **`app/(app)/layout.tsx`** — added `<SiteFooter />` after `{children}`, so the whole logged-in/out consumer nav group now has a footer to match its existing header.
- **`app/page.tsx`** — swapped `LandingFooter` for `SiteFooter`; landing's own header (the "don't show logged-out visitors the full logged-in nav" one from §12a) is untouched.
- **`app/login/page.tsx` / `app/signup/page.tsx`** — added `PublicHeader` + `SiteFooter`, kept the existing "Browse without an account" link (still useful — more prominent than the footer's Browse link).

Verified: `npx eslint --fix` and `npx tsc --noEmit` both clean across all changed files. Visually click-through'd `/login`, `/browse`, and `/` in Chrome against the already-running local dev server (port 3000) — header and footer render correctly on all three, including the full 4-column footer layout. Not re-checked: `/signup`, `/dashboard/*`, `/items/*`, `/messages`, `/profile`, `/messages` — same layout change as `/browse` (shared `(app)` layout) so low risk, but not individually screenshotted. Committed as `ce7acd2` and pushed to `main`.
